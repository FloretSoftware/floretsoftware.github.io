---
title: Unreal Stealth
layout: post
summary: An unreal engine stealth game made to show my learning experience with C++
  in Unreal.
author: floretdev
date: '2024-11-26 17:00:00 +0000'
category: portfolio
thumbnail: "/assets/img/posts/unrealstealth.png"
keywords: unreal engine, c++, AI Programming, gamedev
permalink: "/blog/unreal-stealth/"
usemathjax: true
---

> **Game Engine/Language**: Unreal Engine 5.1 / C++

> **Role**: Solo Dev

> **Duration of Work**: 8 Weeks

> **End Of Project**: 26th November 2024

> **Github Repo**: [Click Here!](https://github.com/Transference-OperatorHaven/Y2C4E) 

Made for my "C++ For Games Engines" Module, I went with a slower pace stealth FPS. This was my most challenging module yet but I still persevere to make a great gameplay experience with it. I learnt about and used a multitude of things but most namely using C++ in unreal and all the interesting niches to make things unique for unreal.

Below is a demo video for the game:

<iframe width="560" height="315" src="https://www.youtube.com/embed/fu9j1CLADvo?si=B2S4pyq5NCNvYww9" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

Below is a screenshot of the C++ code made for the game:

![C++ Code](https://static.wixstatic.com/media/1b2fcd_19085222a7a442f3928d47621835bba3~mv2.png/v1/fit/w_821,h_552,q_90/1b2fcd_19085222a7a442f3928d47621835bba3~mv2.png)

Gorilla Model from https://sketchfab.com/3d-models/caesar-vgdc-2f360d0ff51b4f42b624c98d80edd2c2
